...
def batch_pred(model, processor, audio_paths):
    ...
    for audio_path in audio_paths:
        ...
        encoder_outputs.append(processor(audio=speech_data, sampling_rate=16000,return_tensors="pt")['input_features']) 
    encoder_outputs = torch.cat(encoder_outputs, dim=0).to("cuda")
...
def batch_inference(path, model, processor, batch_size=32):
    model.eval()

    audio_paths = glob(os.path.join(path, '*'))
    results = []

    for i in tqdm(range(0, len(audio_paths), batch_size)):
        batch_audio_paths = audio_paths[i:i + batch_size]
        transcriptions = batch_pred(model, processor, batch_audio_paths)

        for j in range(len(transcriptions)):
            results.append(
                {
                    'filename': batch_audio_paths[j].split('/')[-1],
                    'text': transcriptions[j]
                }
            )
...
def speech_to_batch(hub_dataset,dataset_path,processor):
    ...
    speech_data = speech_data[0]
    #print("speech_data : ",speech_data.shape)
...
def main(args):
    ...
    # train, valid split
    audio_dataset = audio_dataset.train_test_split(test_size=(1000 / max_length),seed=42) #일단은 랜덤하게 다 나눔.
